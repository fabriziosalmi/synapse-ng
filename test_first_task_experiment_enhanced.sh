...
# Initialize metrics (using separate variables instead of associative array for bash 3.2 compatibility)
METRIC_experiment_start=$(get_timestamp_ms)
...
NODE1_ID=$(curl -s http://localhost:8001/whoami | jq -r '.node_id')
NODE2_ID=$(curl -s http://localhost:8002/whoami | jq -r '.node_id')
NODE3_ID=$(curl -s http://localhost:8003/whoami | jq -r '.node_id')
...
METRIC_node1_id=$NODE1_ID
METRIC_node2_id=$NODE2_ID
METRIC_node3_id=$NODE3_ID
...
METRIC_balance1_init=$BALANCE1_INIT
METRIC_balance2_init=$BALANCE2_INIT
...
METRIC_task_creation_start=$(get_timestamp_ms)

TASK_RESPONSE=$(curl -s -X POST "http://localhost:8001/tasks?channel=$CHANNEL" \
...
METRIC_task_creation_end=$(get_timestamp_ms)
...
CREATION_TIME=$((METRIC_task_creation_end - METRIC_task_creation_start))
print_metric "Tempo creazione task: ${CREATION_TIME}ms"

print_header "STEP 4: Verifica Balance Frozen (Creator)"

echo "Attendo 15s per propagazione..."
sleep 15

METRIC_balance_check_start=$(get_timestamp_ms)
...
METRIC_balance1_frozen=$BALANCE1_FROZEN
...
METRIC_balance1_on_node2=$BALANCE1_ON_NODE2
METRIC_balance1_on_node3=$BALANCE1_ON_NODE3
...
if [ "$BALANCE1_FROZEN" -eq "$BALANCE1_ON_NODE2" ] && [ "$BALANCE1_FROZEN" -eq "$BALANCE1_ON_NODE3" ]; then
    ...
    METRIC_checkpoint1="PASS"
else
    print_error "❌ CHECKPOINT 1 FALLITO: Consenso mancante"
    METRIC_checkpoint1="FAIL"
    exit 1
fi

# Misura tempo di propagazione
METRIC_task_propagation_check=$(get_timestamp_ms)
PROPAGATION_TIME=$((METRIC_task_propagation_check - METRIC_task_creation_end))
print_metric "Tempo propagazione task creation: ${PROPAGATION_TIME}ms"

print_header "STEP 5: Claim Task (Nodo 2 = Contributor)"

wait_for_condition "curl -s http://localhost:8002/state | jq -e '.$CHANNEL.tasks.\"$TASK_ID\"'" "Task visibile su Nodo 2" 30

METRIC_claim_start=$(get_timestamp_ms)

CLAIM_RESPONSE=$(curl -s -X POST "http://localhost:8002/tasks/$TASK_ID/claim?channel=$CHANNEL")

METRIC_claim_end=$(get_timestamp_ms)

echo "$CLAIM_RESPONSE" | jq .

print_success "Task claimed dal Nodo 2"

CLAIM_TIME=$((METRIC_claim_end - METRIC_claim_start))
print_metric "Tempo operazione claim: ${CLAIM_TIME}ms"

echo ""
print_info "Attendo 15s per propagazione claim..."
sleep 15

METRIC_claim_propagation_check=$(get_timestamp_ms)

# Verifica che tutti i nodi vedono il claim
STATUS_NODE1=$(curl -s http://localhost:8001/state | jq -r ".$CHANNEL.tasks.\"$TASK_ID\".status")
STATUS_NODE2=$(curl -s http://localhost:8002/state | jq -r ".$CHANNEL.tasks.\"$TASK_ID\".status")
STATUS_NODE3=$(curl -s http://localhost:8003/state | jq -r ".$CHANNEL.tasks.\"$TASK_ID\".status")
...
METRIC_status_node1=$STATUS_NODE1
METRIC_status_node2=$STATUS_NODE2
METRIC_status_node3=$STATUS_NODE3
...
if [ "$STATUS_NODE1" = "claimed" ] && [ "$STATUS_NODE2" = "claimed" ] && [ "$STATUS_NODE3" = "claimed" ]; then
    ...
    METRIC_checkpoint2="PASS"
else
    print_error "❌ CHECKPOINT 2 FALLITO: Status inconsistente"
    METRIC_checkpoint2="FAIL"
    exit 1
fi

CLAIM_PROPAGATION_TIME=$((METRIC_claim_propagation_check - METRIC_claim_end))
...
# Progress
METRIC_progress_start=$(get_timestamp_ms)
curl -s -X POST "http://localhost:8002/tasks/$TASK_ID/progress?channel=$CHANNEL" > /dev/null
METRIC_progress_end=$(get_timestamp_ms)
print_success "Task in progress"

sleep 5

# Complete
METRIC_complete_start=$(get_timestamp_ms)
curl -s -X POST "http://localhost:8002/tasks/$TASK_ID/complete?channel=$CHANNEL" > /dev/null
METRIC_complete_end=$(get_timestamp_ms)
print_success "Task completato"

COMPLETE_TIME=$((METRIC_complete_end - METRIC_complete_start))
...
METRIC_complete_propagation_check=$(get_timestamp_ms)
...
METRIC_balance1_final=$BALANCE1_FINAL
METRIC_balance2_final=$BALANCE2_FINAL

CREATOR_DELTA=$((BALANCE1_FINAL - BALANCE1_INIT))
CONTRIBUTOR_DELTA=$((BALANCE2_FINAL - BALANCE2_INIT))

METRIC_creator_delta=$CREATOR_DELTA
METRIC_contributor_delta=$CONTRIBUTOR_DELTA
...
METRIC_balance2_on_node1=$BALANCE2_ON_NODE1
METRIC_balance2_on_node3=$BALANCE2_ON_NODE3

COMPLETE_PROPAGATION_TIME=$((METRIC_complete_propagation_check - METRIC_complete_end))
...
METRIC_experiment_end=$(get_timestamp_ms)
TOTAL_TIME=$((METRIC_experiment_end - METRIC_experiment_start))
...
printf "${CYAN}│${NC} %-45s ${MAGENTA}%10s${NC} ${CYAN}│${NC}\n" "Checkpoint 1 (Balance Frozen)" "${METRIC_checkpoint1}"
printf "${CYAN}│${NC} %-45s ${MAGENTA}%10s${NC} ${CYAN}│${NC}\n" "Checkpoint 2 (Task Claimed)" "${METRIC_checkpoint2}"
printf "${CYAN}│${NC} %-45s ${MAGENTA}%10s${NC} ${CYAN}│${NC}\n" "Checkpoint 3 (Reward Transfer)" "${METRIC_checkpoint3}"
echo -e "${CYAN}└─────────────────────────────────────────────────────────────────┘${NC}"
...
cat > "$METRICS_FILE" <<EOF
{
  ...
  "consensus": {
    ...
    "checkpoint1": "${METRIC_checkpoint1}",
    "checkpoint2": "${METRIC_checkpoint2}",
    "checkpoint3": "${METRIC_checkpoint3}"
  },
  "node_ids": {
    "node1": "$NODE1_ID",
    "node2": "$NODE2_ID",
